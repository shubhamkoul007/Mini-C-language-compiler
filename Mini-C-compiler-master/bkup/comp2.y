...
br_list : LSQBRAC expression RSQBRAC { 
        ...
        | LSQBRAC expression RSQBRAC br_list { 
            $$.dim_list = (int*)malloc(sizeof(int)*($4.size+1));

            int iter = 0;
            for(iter = 0;iter < $4.size; ++iter) $$.dim_list[iter] = $4.dim_list[iter];
            

            $$.size = $4.size + 1;

            if($2.et == INT_T && $2.ptr_depth == 0 && $2.size == 0 && $2.expr_val > 0){
                $$.dim_list[$4.size] = $2.expr_val;
            }
            else{ 
                printf("index must be int and +ve constant\n");
                $$.dim_list[$4.size] = 0;
...
void patch_type(struct node type,struct list_attr lis){
    ...
    for(iter = 0;iter < lis.size; ++iter){
        if(exists_in_scope(lis.list_var[iter])){
            ...
            if(lis.list_var[iter].size == 0){
                lis.list_var[iter].st_off = cur;
            }
